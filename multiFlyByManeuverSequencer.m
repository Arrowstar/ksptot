...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if(isempty(errMsg))
        ...
        cBodyInfo = celBodyData.(lower(cbName));
        ...
        [x, dv, rp, orbitsIn, orbitsOut, xferOrbits, deltaVVect, vInfDNorm, c, vInfDepart, vInfArrive] = ...
            multiFlybyExec(wayPtBodies,lWindDef,bnds,popSize,numGen,celBodyData);

        fun = @(x) computeDepartArriveDVFromEllipticTarget(eSMA, eEcc, eInc, eRAAN, eArg, x, wayPtBodies{1}.gm, vInfDepart);
        nonlcon = @(x) hyperOrbitExcessVelConst(eSMA, eEcc, eInc, eRAAN, eArg, x, wayPtBodies{1}.gm, vInfDepart, 1);

        [eTA,~] = multiStartCommonRun('Searching for departure orbit...',...
                                       25,...
                                       fun, pi, [], [], 0, 2*pi, nonlcon);
        [~, dVDepartVect, dVDepartVectNTW, eRDepartVect, hOrbit, ~, ~] = fun(eTA);
        ...
        outputs = {wayPtBodies, x, dv, rp, orbitsIn, orbitsOut, xferOrbits, deltaVVect, vInfDNorm, c, vInfDepart, dVDepartVect, dVDepartVectNTW, eRDepartVect, eOrbit, hOrbit, cBodyInfo, deltaVVectNTW};
        ...
        form = '%9.3f';
        paddLen = 32;
        printHyperOrbitsMFMSToTextbox(handles.hyperbolicOrbitsText, wayPtBodies, hOrbit, orbitsIn, orbitsOut, vInfArrive, form, paddLen);
        printXfrOrbitsMFMSToTextbox(handles.transferOrbitsText,wayPtBodies, xferOrbits, form, paddLen);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    userData = get(handles.multiFlybyManSeqGUI, 'UserData');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    orbitPanelGetOrbitFromKSPTOTConnectCallBack(handles.eSMAText, handles.eEccText, handles.eIncText, handles.eRAANText, handles.eArgText);
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
    ...
    if(value == 1)
        plotAllFlybys(handles.dispAxesMFMS, cBodyInfo, xferOrbits, wayPtBodies, celBodyData);
