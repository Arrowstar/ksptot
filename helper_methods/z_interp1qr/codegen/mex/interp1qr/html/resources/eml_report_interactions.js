...
// CSS selector for the currently selected potential difference message.
var currentPotentialDiff = "";
...
function setupInteractions() {
    ...
    $(document)
        .bind('keydown', keyBindings.selectMCodeTab, function(evt) {
            selectMCodeTab();
            return false;
        })
        .bind('keydown', keyBindings.selectCallStackTab, function(evt) {
            selectCallStackTab();
            return false;
        })
        .bind('keydown', keyBindings.selectCCodeTab, function(evt) {
            selectCCodeTab();
            return false;
        })

    .bind('keydown', keyBindings.selectReportTab, function(evt) {
            selectReportTab();
            return false;
        })
        .bind('keydown', keyBindings.selectCenterPanel, function(evt) {
            selectCenterPanel();
            return false;
        })

    .bind('keydown', keyBindings.selectSummaryTab, function(evt) {
            selectSummaryTab();
            return false;
        })
        .bind('keydown', keyBindings.selectMessagesTab, function(evt) {
            selectMessagesTab();
            return false;
        })
        .bind('keydown', keyBindings.selectWatchListTab, function(evt) {
            selectWatchListTab();
            return false;
        })
        .bind('keydown', keyBindings.selectBuildLogTab, function(evt) {
            selectBuildLogTab();
            return false;
        })

    .bind('keydown', keyBindings.clearAllHighlights, function(evt) {
        clearAllHighlights();
        return false;
    });
}
...
function loadWatchData(id) {
    ...
    if (watchListTabSelected()) {
        ...
        try {
            ...
            loadExternalWatchData(watchFilename, function() {
                setupWatchList();
                initializeWatchList();
...
function loadFunctionData(id, pieces, postLoadCallback) {
    try {
        // AJAX load.
        ...
        loadExternalCode(mcodeFilename, function() {
            ...
            clearPotentialDifferenceHighlights();
            highlightCurrentPotentialDifference();
...
function makeCTitle(filename) {
    // Supported file extensions: .c, .cpp, .h, .cu
    var re = /_[cphu]{1,3}.html$/;
...
function loadCData(filename, pieces, isUnderWarning, postLoadCallback) {
    try {
        ...
        if (isUnderWarning) {
            ...
            loadExternalCode(filename, function() {
                initializeCData(filename, isUnderWarning);
                if (pieces.length > 1) {
                    // Scroll the file if requested.
...
function loadReportData(filename, pieces, postLoadCallback) {
    try {
...
function loadExternalCode(filename, handler, selector) {
    loadExternal(queries.codeDiv, filename, handler, selector);
}

function loadExternalWatchData(filename, handler, selector) {
    loadExternal(queries.watchlist, filename, handler, selector);
}

function loadExternal(element, filename, handler, selector) {
    function loadUsingIFrame() {
        window.addEventListener("message", function messageListener(e) {
            console.log("Message received.");
            if (typeof selector === "undefined") {
                element.html(e.data);
            } else {
                element.html($(selector, e.data));
            }
        ...
        $("#hiddenCodeFrame").attr('src', filename);
    }

    try {
        // Perform an AJAX load.
        var ajaxSelector = filename;
        if (typeof selector !== "undefined") {
            ajaxSelector += " " + selector;
        }
        element.load(ajaxSelector, function(responseText, textStatus, xhr) {
            if (textStatus === "error" || !responseText) {
                // AJAX failed, fallback to using an iframe to load.
                loadUsingIFrame();
...
function openMexWarning() {
    loadExternalCode("mex_warning.html");
}
...
function openFunction(aId, aOptions, aPostLoadCallback) {
    ...
    if (aPostLoadCallback === undefined) {
        aPostLoadCallback = function() {};
    }
    // Parse the id
    ...
    if (intId <= 0) {
        return;
    } else if (activeFcn.id == id && intId == id) {
    ...
    if (intId == id) {
        ...
        unselectCCodeTab();
        loadFunctionData(id, pieces, aPostLoadCallback);
        setWatchListTabVisible(true);
    } else if (aOptions.tabId === "report") {
        // Load a report file
        selectCCodeTab();
        loadReportData(id, pieces, aPostLoadCallback);
        ...
        openMexWarning();
        loadCData(id, pieces, aOptions.isUnderWarning, aPostLoadCallback);
        setWatchListTabVisible(false);
    } else {
...
// Opens a function if no MATLAB function is being loaded
    ...
    $("a",queries.codeDiv).each(function() {
        if (this.getAttribute("href") !== null) {
            ...
            if ($this.hasClass("code2code")) {
                var hrefStr = $this.attr("href");
                str = hrefStr.substr(hrefStr.lastIndexOf("/") + 1);
                $this
                    .attr("fcnid", str)
                    .click(function(evt) {
                        queries.ccoderows.removeClass("fcnid-hover");
...
function showTooltip(evt, text) {
    ...
    c = tooltip.counter + 1;
    tooltip.counter = c;
    $(evt.target).bind('mouseleave', function(evt) {
        hideTooltip(evt, c);
    });
...
function refreshShowOptHighlight() {
    ...
    toggleFcnsWithViolations($("#majoritytransposeenable"), $(".majoritytranspose"), "majorityTransposeHighlight");
    toggleFcnsWithViolations($("#majorityindexingenable"), $(".majorityindexing"), "majorityIndexingHighlight");
}

function clearExpensiveOperationHighlights() {
    var highlightCategories = document.getElementsByName("highlightCategory");
    for (var i = 0; i < highlightCategories.length; i++) {
        if (highlightCategories[i].checked) {
            highlightCategories[i].checked = false;
        }
    }
    refreshShowOptHighlight();
}

// Toggle the highlighting of a class of issues e.g. row major, double/single/fixed-point operations.
...
function fixptRadioClick(evt) {
    refreshShowOptHighlight();
}

// Click on expensive majority transposes will trigger function names
// with expensive majority operations will be highlighted
function majorityTransposeRadioClick(evt) {
    refreshShowOptHighlight();
}

// Click on expensive majority indexing will trigger function names
// with expensive majority indexing will be highlighted
function majorityIndexingRadioClick(evt) {
    refreshShowOptHighlight();
}
...
function messagesClick(evt) {
    var $obj = getMessageClickTarget(evt);
...
function clickPotentialDifferenceMessage(evt) {
    var $obj = getMessageClickTarget(evt);
    currentPotentialDiff = $obj.attr("id");

    var intIds = decodeFunctionID($(".fcn." + currentPotentialDiff).attr("id"));
    openFunction(intIds[1], defaultOpenFunctionOptions(), function() {
        clearPotentialDifferenceHighlights();
        highlightPotentialDifference(currentPotentialDiff);
        scrollToObj(queries.codeDiv, $("." + currentPotentialDiff, queries.codeDiv));
    });
}

function highlightCurrentPotentialDifference() {
    if (currentPotentialDiff.trim()) {
        highlightPotentialDifference(currentPotentialDiff);
    }
}

function highlightPotentialDifference(pd) {
    $("." + pd).addClass("potentialDiffHighlight");
    $("#" + pd).addClass("potentialDiffHighlight");
}

function clearPotentialDifferenceHighlights() {
    $(".potentialDiffHighlight").removeClass("potentialDiffHighlight");
}
...
function clearAllHighlights() {
    ...
    currentPotentialDiff = "";

    clearExpensiveOperationHighlights();
}
