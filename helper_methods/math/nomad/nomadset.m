...
global dirtypes intPollDirTypes
...
intPollDirTypes = {'ORTHO 1', 'GPS 1', 'LT 1'};
...
Names = {'bb_input_type','bb_output_type','direction_type','f_target','initial_mesh_size','lh_search','max_bb_eval','bb_max_block_size',...
         ...
         'sgte_cache_file','max_sgte_eval','anisotropic_mesh','fixed_variable','intensification_type','max_eval_intensification','int_poll_dir_types','granularity','nm_search'}';
...
switch lower(field)   
    ...
    case {'int_poll_dir_types'}
        if(~ischar(value) || ~any(strcmp(value,intPollDirTypes)))
            err = MException('NOMAD:SetFieldError','Parameter ''%s'' should match one of those displayed by nomadset()',field);
        end  
