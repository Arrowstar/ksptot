function [D,allE] = is_intrinsic_delaunay(V,F,varargin)
  % IS_INTRINSIC_DELAUNAY Determine if each edge in F is *intrinsically* Delaunay. 
  %
  % Inputs:
  ...
  % Known bugs: locally intrinsically Delaunay (as defined by [Fisher et al.
  %   2007]) is not the same as locally Delaunay in 2D. Consider a non-convex
  %   quad. A flipped triangle implies a non-locally Delaunay edge, but a
  %   flipped triangle does not necessarily imply a non-locally-intrinsically
  %   Delaunay edge (all cotans can be positive).
  %
