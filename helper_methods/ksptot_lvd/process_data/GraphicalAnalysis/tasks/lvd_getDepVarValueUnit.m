function [depVarValue, depVarUnit, taskStr, refBodyInfo] = lvd_getDepVarValueUnit(i, subLog, taskStr, refBodyId, celBodyData, onlyReturnTaskStr, inFrame)
    ...
    switch taskStr
        ...
        otherwise %is a programmatically generated string that we'll handle here
            ...
            elseif(not(isempty(regexpi(taskStr, pluginValuePattern))))
                tokens = regexpi(taskStr, pluginValuePattern, 'tokens');
                tokens = tokens{1};
                tokens = tokens{1};
                pluginInd = str2double(tokens);
                
                lvdData = subLog(i).lvdData;
                [~, plugins] = lvdData.plugins.getAllPluginGraphAnalysisTaskStrs();
                plugin = plugins(pluginInd);
