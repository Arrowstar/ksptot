classdef TotalBodyAngularVelConstraint < AbstractConstraint
    %TotalBodyAngularVelConstraint Summary of this class goes here
    ...
    methods
        function obj = TotalBodyAngularVelConstraint(event, lb, ub)
            obj.event = event;
            obj.lb = lb;
            obj.ub = ub;  
        ...
        function [c, ceq, value, lwrBnd, uprBnd, type, eventNum, valueStateComp] = evalConstraint(obj, stateLog, celBodyData)           
            ...
            value = lvd_AttitudeRateTasks(stateLogEntry, 'totalAngularVel', obj.frame);
        ...
        function type = getConstraintType(obj)
            type = 'Total Body Angular Velocity';
        end
    ...
    methods(Static)
        function constraint = getDefaultConstraint(~, ~)            
            constraint = TotalBodyAngularVelConstraint(LaunchVehicleEvent.empty(1,0),0,0);
        end
    end
end