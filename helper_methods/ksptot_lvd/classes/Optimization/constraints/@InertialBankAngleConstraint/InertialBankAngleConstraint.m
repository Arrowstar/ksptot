classdef InertialBankAngleConstraint < AbstractConstraint
    %InertialBankAngleConstraint Summary of this class goes here
    ...
    methods
        function obj = InertialBankAngleConstraint(event, lb, ub)
            obj.event = event;
            obj.lb = lb;
            obj.ub = ub;   
        ...
        function [c, ceq, value, lwrBnd, uprBnd, type, eventNum, valueStateComp] = evalConstraint(obj, stateLog, celBodyData)           
            ...
            [inertBankAng,~,~] = stateLogEntry.attitude.getInertialAeroAngles(ut, rVect, vVect, bodyInfo);
            value = rad2deg(inertBankAng);
            ...
            if(obj.evalType == ConstraintEvalTypeEnum.StateComparison)
                ...
                [inertBankAng,~,~] = stateLogEntryStateComp.attitude.getInertialAeroAngles(ut, rVect, vVect, bodyInfo);
                valueStateComp = rad2deg(inertBankAng);
            else
                valueStateComp = NaN;
            end
...
%         end
    ...
    methods(Static)
        function constraint = getDefaultConstraint(~, ~)            
            constraint = InertialBankAngleConstraint(LaunchVehicleEvent.empty(1,0),0,0);
        end
    end
end