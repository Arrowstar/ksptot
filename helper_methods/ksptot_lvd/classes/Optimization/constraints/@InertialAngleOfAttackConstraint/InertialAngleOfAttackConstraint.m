classdef InertialAngleOfAttackConstraint < AbstractConstraint
    %InertialAngleOfAttackConstraint Summary of this class goes here
    ...
    methods
        function obj = InertialAngleOfAttackConstraint(event, lb, ub)
            obj.event = event;
            obj.lb = lb;
            obj.ub = ub;     
        ...
        function [c, ceq, value, lwrBnd, uprBnd, type, eventNum, valueStateComp] = evalConstraint(obj, stateLog, celBodyData)           
            ...
            [~,inertAngOfAttack,~] = stateLogEntry.attitude.getInertialAeroAngles(ut, rVect, vVect, bodyInfo);
            value = rad2deg(inertAngOfAttack);
            ...
            if(obj.evalType == ConstraintEvalTypeEnum.StateComparison)
                ...
                [~,inertAngOfAttack,~] = stateLogEntryStateComp.attitude.getInertialAeroAngles(ut, rVect, vVect, bodyInfo);
                valueStateComp = rad2deg(inertAngOfAttack);
            else
                valueStateComp = NaN;
            end
...
%         end
    ...
    methods(Static)
        function constraint = getDefaultConstraint(~, ~)            
            constraint = InertialAngleOfAttackConstraint(LaunchVehicleEvent.empty(1,0),0,0);
        end
    end
end