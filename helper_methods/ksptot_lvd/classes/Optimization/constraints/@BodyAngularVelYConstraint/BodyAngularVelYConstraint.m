classdef BodyAngularVelYConstraint < AbstractConstraint
    %BodyAngularVelYConstraint Summary of this class goes here
    ...
    methods
        function obj = BodyAngularVelYConstraint(event, lb, ub)
            obj.event = event;
            obj.lb = lb;
            obj.ub = ub;  
        ...
        function [c, ceq, value, lwrBnd, uprBnd, type, eventNum, valueStateComp] = evalConstraint(obj, stateLog, celBodyData)           
            ...
            value = lvd_AttitudeRateTasks(stateLogEntry, 'bodyAngVelY', obj.frame);
            ...
            if(obj.evalType == ConstraintEvalTypeEnum.StateComparison)
                ...
                valueStateComp = lvd_AttitudeRateTasks(stateLogEntryStateComp, 'bodyAngVelY', obj.frame);
            else
                valueStateComp = NaN;
            end
        ...
        function type = getConstraintType(obj)
            type = 'Body Y-axis Angular Velocity';
        end
    ...
    methods(Static)
        function constraint = getDefaultConstraint(~, ~)            
            constraint = BodyAngularVelYConstraint(LaunchVehicleEvent.empty(1,0),0,0);
        end
    end
end