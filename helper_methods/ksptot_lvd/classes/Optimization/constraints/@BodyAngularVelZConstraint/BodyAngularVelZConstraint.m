classdef BodyAngularVelZConstraint < AbstractConstraint
    %BodyAngularVelZConstraint Summary of this class goes here
    ...
    methods
        function obj = BodyAngularVelZConstraint(event, lb, ub)
            obj.event = event;
            obj.lb = lb;
            obj.ub = ub;  
        ...
        function [c, ceq, value, lwrBnd, uprBnd, type, eventNum, valueStateComp] = evalConstraint(obj, stateLog, celBodyData)           
            ...
            value = lvd_AttitudeRateTasks(stateLogEntry, 'bodyAngVelZ', obj.frame);
            ...
            if(obj.evalType == ConstraintEvalTypeEnum.StateComparison)
                ...
                valueStateComp = lvd_AttitudeRateTasks(stateLogEntryStateComp, 'bodyAngVelZ', obj.frame);
            else
                valueStateComp = NaN;
            end
        ...
        function type = getConstraintType(obj)
            type = 'Body Z-axis Angular Velocity';
        end
    ...
    methods(Static)
        function constraint = getDefaultConstraint(~, ~)            
            constraint = BodyAngularVelZConstraint(LaunchVehicleEvent.empty(1,0),0,0);
        end
    end
end