classdef AdamNlOptOptimizer < AbstractGradientOptimizer
    %AdamNlOptOptimizer LVD wrapper around the AdamNLOpt nonlinear programming
    %solver (adamnlopt.solve).
    %   adamnlopt.solve is a drop-in fmincon replacement, so this class follows
    %   FminconOptimizer almost step for step.  The two places it must differ
    %   are the options struct (a plain struct rather than an optimoptions
    %   object) and the progress/cancel hook (adamnlopt has a single
    %   IterationFcn(info) callback rather than fmincon's OutputFcn).
    ...
    methods
        ...
        function [exitflag, message] = optimize(obj, lvdOpt, writeOutput, callOutputFcn, hLvdMainGUI)
            ...
            %Gradient wiring mirrors FminconOptimizer.  On BuiltIn we hand
            %adamnlopt bare function handles and let it do its own
            %differencing, which is what keeps its autoFDStep noise
            %calibration in play -- that calibration is the main reason to
            %prefer this solver on LVD's ODE-propagated problems.
            if(lvdOpt.gradAlgo == LvdOptimizerGradientCalculationAlgoEnum.BuiltIn)
                ...
                opts.SpecifyConstraintGradient = false;
            ...
            %Keep variables off their bounds on the initial guess, same as the
            %other optimizers do.
            bool = x0All >= ubAll;
            ...
            %Build the problem struct by hand: createOptimProblem only knows
            %the MATLAB solvers.  SQPOptimizer takes the same approach.
            problem = struct();
            problem.objective = objFunToPass;
            problem.x0 = x0All;
            problem.Aineq = [];
            problem.bineq = [];
            problem.Aeq = [];
            ...
            problem.solver = 'adamnlopt';
            problem.lvdData = lvdOpt.lvdData; %need to get lvdData in somehow

            %lvd_executeOptimProblem looks for problem.options.UseParallel
            %first and falls back to problem.UseParallel; the adamnlopt options
            %struct has no UseParallel field, so the fallback is what gets read.
        ...
        function openOptionsDialog(obj)
            ...
            lvd_editAdamNlOptOptionsGUI_App(obj, output);
        end

        function tf = usesParallel(obj)
            tf = obj.options.usesParallel();
        end
    ...
    methods(Static, Access=private)
        function stop = iterationFunc(info, outputFnc, maxTime, optimStartTic)
            ...
            %
            %   NOTE ON THE RETURN VALUE: adamnlopt stops the solve when this
            %   returns TRUTHY, so stop is passed through un-inverted.  This is
            %   the opposite of IpOptOptimizer.outputFunc, whose trailing
            %   "stop = not(logical(stop))" exists because IPOPT's iterfunc
            %   returns "keep going".  Inverting here would make Cancel do
            ...
            if(not(isempty(outputFnc)))
                optimValues = struct('iteration', info.iteration, ...
                                     ...
                                     'fval', info.fval, ...
                                     'constrviolation', info.constrviolation, ...
                                     'firstorderopt', info.firstorderopt, ...
                                     'stepsize', info.stepsize, ...
                                     'procedure', '');

                %info.x is already unscaled and indexed over the original
                %variables, so it feeds the recorder and plots directly.
                if(info.iteration == 0)
                    states = {'init','iter'};
                else
                    states = {'iter'};
                ...
                for(i=1:length(states))
                    stopOut = outputFnc(info.x, optimValues, states{i});
                    stop = stop || logical(stopOut);
                end
            end
        ...
        function stop = getOutputFunction(x, optimValues, state, hOptimStatusLabel, hFinalStateOptimLabel, hDispAxes, hCancelButton, ...
            ...
            if(strcmpi(state,'init') || strcmpi(state,'iter'))
                try
                    AdamNlOptOptimizer.writeOptimStatus(hOptimStatusLabel, optimValues, state, writeOutput, optimStartTic);
