    ...
    methods
        ...
        function [exitflag, message] = optimize(obj, lvdOpt, writeOutput, callOutputFcn, hLvdMainGUI, progressFcn)
            ...
            elseif(not(isempty(progressFcn)))
                ...
                problem.options.OutputFcn = @(x, optimValues, state) SQPOptimizer.reportHeadlessProgress(optimValues, state, progressFcn);
            end
        ...
        function stop = getOutputFunction(x, optimValues, state, hOptimStatusLabel, hFinalStateOptimLabel, hDispAxes, hCancelButton, ...
            ...
            if(strcmpi(state,'init') || strcmpi(state,'iter'))
                try
                    ...
                    SQPOptimizer.generatePlots(x, optimValues, state, hDispAxes, lb, ub, varLabels, lbUsAll, ubUsAll, recorder);
                    drawnow;
                catch ME
                    warning(ME.message);
                end
