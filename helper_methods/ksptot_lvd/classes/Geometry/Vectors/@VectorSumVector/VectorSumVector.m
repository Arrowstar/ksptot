classdef VectorSumVector < AbstractGeometricVector
    %VectorSumVector Output is the sum of two input vectors (vector1 + vector2).
    ...
    methods        
        function obj = VectorSumVector(vector1, vector2, name, lvdData) 
            obj.vector1 = vector1;
            obj.vector2 = vector2;
        ...
        function listboxStr = getListboxStr(obj)
            listboxStr = sprintf('%s ("%s" plus "%s")', obj.getName(), obj.vector1.getName(), obj.vector2.getName());
        end
        
        function useTf = openEditDialog(obj)            
            output = AppDesignerGUIOutput({false});
            lvd_EditVectorSumVectorGUI_App(obj, obj.lvdData, output);
        ...
        function origin = getOriginPointInViewFrame(~, ~, ~, ~)
            origin = [0;0;0];
        end
