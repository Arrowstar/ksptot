classdef ODE23sIntegrator < AbstractFirstOrderIntegrator
    %ODE23Integrator Summary of this class goes here
    %   Detailed explanation goes here
    
    properties
        options(1,1) BuiltInIntegratorOptions = BuiltInIntegratorOptions();
        integratorEnum = IntegratorEnum.ODE23s;
    end
    
    methods
        function obj = ODE23sIntegrator(options)
            if(nargin > 0)
                obj.options = options;
            else
        ...
        function [t,y,te,ye,ie] = integrate(obj, odefun, tspan, y0, evtsFunc, odeOutputFun)
            ...
            [t,y,te,ye,ie] = ode23s(odefun, tspan, y0, optionsToUse);
        end
