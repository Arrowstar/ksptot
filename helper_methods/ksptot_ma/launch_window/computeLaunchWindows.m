...
%   Detailed explanation goes here
    ...
    beta2 = getLaunchAz(x1, sinBeta, dHVect, distFunc);
    beta1 = getLaunchAz(x2, sinBeta, dHVect, distFunc);
end
...
function beta = getLaunchAz(x, sinBeta, hVect, distFunc)
    [~,rVect] = distFunc(x);
    vVect = cross(hVect,rVect);
    beta = asin(sinBeta);
    if(vVect(3) < 0)
        beta = pi - beta;
    end
