...
%     dataLength = 17;
    ...
    for(i=1:length(bodiesData))
        ...
        bodystr = assemBodyDataStr(bodyI.name, bodyI.epoch, bodyI.sma, bodyI.ecc, bodyI.inc, bodyI.raan, bodyI.arg, ...
                                   ...
                                   bodyI.arrays{1,6}, bodyI.arrays{2,6},...
                                   bodyI.arrays{1,7}, bodyI.arrays{2,7},...
                                   bodyI.arrays{1,8}, bodyI.arrays{2,8},...
                                   bodyI.atmoMolarMass);
...
function bodystr = assemBodyDataStr(name, epoch, sma, ecc, inc, raan, arg, mean, gm, radius, atmoHgt, ...
                                    ...
                                    axialTemperatureSunBiasDeg, axialTemperatureSunBiases, ...
                                    axialTemperatureSunMultLats, axialTemperatureSunMults, ...
                                    eccentricityTemperatureBiasPts, eccentricityTemperatureBiases, ...
                                    atmoMolarMass)
    ...
    bodystr = sprintf(['[%s]\nepoch = %0.9f\nsma = %0.9f\necc = %0.9f\ninc = %0.9f\nraan = %0.9f\narg = %0.9f\nmean = %0.9f\ngm = %0.9f\nradius = %0.9f\natmoHgt = %0.9f\n',...
                       ...
                       'axialTemperatureSunBiasDeg = %s\naxialTemperatureSunBiases = %s\n',...
                       'axialTemperatureSunMultLats = %s\naxialTemperatureSunMults = %s\n',...
                       'eccentricityTemperatureBiasPts = %s\neccentricityTemperatureBiases = %s\n',...
                       'atmoMolarMass = %0.9f\n',...
                       'rotperiod = %0.9f\nrotini = %0.9f\nbodycolor = gray\ncanBeCentral = %i\ncanBeArriveDepart = %i\nparent = %s\nparentID = %i\nname = %s\nid = %i\n\n'], ...
