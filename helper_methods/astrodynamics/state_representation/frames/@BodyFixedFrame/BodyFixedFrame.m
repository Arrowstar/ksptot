    ...
    methods
        ...
        function R_BodyFixed_to_GlobalInertial = getRotMatToInertialAtTime(obj, time, ~, ~)
            if(isscalar(time) && obj.rotMatToInertialCacheTime == time)
                ...
                R_BodyFixed_to_GlobalInertial = getBodyFixedToGlobalInertialFrame_mex(time, spinAngle, bi.bodyRotMatFromGlobalInertialToBodyInertial);
                % R_BodyFixed_to_GlobalInertial = getBodyFixedToGlobalInertialFrame(time, spinAngle, bi.bodyRotMatFromGlobalInertialToBodyInertial);
