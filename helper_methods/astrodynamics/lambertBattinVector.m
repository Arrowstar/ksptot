...
%             y = nan(size(h1));
%             while(any(isnan(y)))
%                 y = bisection(yEqn,-bndFactor*ones(size(h1)),bndFactor*ones(size(h1)));
%                 bndFactor = bndFactor*10;
%             end
    ...
    if(any(a(a > 0.0)))
        ...
        betaE(bool) = 2*asin(sinBetaEOver2(bool));
        ...
        alphaE(bool) = 2*asin(sqrt(s(bool)./(2*a(bool))));
        ...
        f(bool)     = 1 - (a(bool)./r1Mag(bool)) .* (1 - cos(deltaE(bool)));
        g(bool)     = dt(bool) - sqrt(a(bool).^3./gmu(bool)) .* (deltaE(bool) - sin(deltaE(bool)));
        g_dot(bool) = 1 - (a(bool)./r2Mag(bool)) .* (1- cos(deltaE(bool)));  
    end
    if(any(a(a < 0.0)))
        bool = a < 0.0;
        ...
        f(bool)     = 1 - (a(bool)./r1Mag(bool)) .* (1 - cosh(deltaH(bool)));
        g(bool)     = dt(bool) - sqrt(-(a(bool).^3)./gmu(bool)) .* (sinh(deltaH(bool)) - deltaH(bool));
        g_dot(bool) = 1 - (a(bool)./r2Mag(bool)) .* (1 - cosh(deltaH(bool)));
    end
    if(any(a(a == 0.0)))
        error('a = 0.0');
    end
