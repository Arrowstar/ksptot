...
**F2. Additional GA quantities.** ✅ **Implemented 2026-09-25** (all but the D2/D5-conditioned sub-items; in the working tree).
*Status: 23 new GA quantities in the existing task registry (`lvd_getGraphAnalysisTaskList`, `lvd_getDepVarValueUnit` dispatcher, no UI changes needed): specific orbital energy; specific angular momentum (magnitude + X/Y/Z); argument of latitude; true longitude; periapsis/apoapsis latitude/longitude; sensed acceleration total/axial/normal in g from thrust + drag + lift; remaining ΔV capability (rocket equation on full-throttle active-engine Isp, coast-stable); cumulative ΔV expended (finite-burn `g0·Isp·ln(m1/m2)` per step plus impulsive `‖Δv‖` on same-time action entries, with a dry-mass-change staging guard); per-engine thrust (kN) / Isp (s) / mass flow (mT/s, outflow-negative, 0/0 when off with Isp held at the pressure-curve value); per-ground-object range rate, elevation rate (analytic chain rule including NED-frame planetary rotation, which is ~0.017°/s on Kerbin and not negligible) and downrange distance; sun phase angle with vertex at the S/C. Decisions taken during implementation: cumulative includes impulsive with per-event/per-stage splits via StateComparison instead of split tasks; downrange origin is a ground object; engine strings keep block order so existing Active-State indices are unchanged. The 14 fixed-string quantities are also `GenericMAConstraint`/`GenericObjectiveFcn` types (`ConstraintEnum` + `ma_getConstraintStaticDetails`); cumulative integrates over the full log up to the constrained node (both nodes, StateComparison included) via a history special-case, and `GraphicalAnalysisTask.executeTask` threads optional log history (plots, sweep responses, overlays) with a sequential-access memo. Tests: `F2GraphicalAnalysisTest` (17 methods: closed forms, a transcription of Vallado Algorithm 9 against his published Example 2-5 vector plus the exact-angle ex2_5.m constructions u = 45°/ω = 20°/l = 65° prograde and retrograde, an elements-free eccentricity-vector apsis check, finite-difference rate checks, and a Hohmann end-to-end where remaining-drop matches cumulative) and the `CaseMatrixPluginGroundObjTest` composition check updated (4→7 tasks per ground object). Deferred as the report conditions them: penumbra fraction (needs D5), heating indicators (needs D2). Known boundaries: dynamic per-object families plot/sweep/overlay but are not `GenericMAConstraint` types (the static-details switch can't enumerate them; dedicated classes or prefix matching remain future work); GA-quantity termination needs A2; cumulative reports 0 in single-entry contexts (action conditionals during propagation, extrema).*
...
**F8. 3-D view: playback, video export, camera follow, colour-by-quantity.** ✅ **Implemented 2026-09-17** (colour-by-quantity deliberately omitted at the user's request).
*Status: a new programmatic window, **View > 3-D View Playback, Camera and Vehicle Mesh...** (`lvd_ViewPlaybackGUI_App.m`, no new `.mlapp`; the menu item is a real canvas component added with `lvdfixCanvasizeF8`), drives the main window's 3-D axes and time slider. **Playback:** play / pause / stop / frame step / go-to-start/end, speed in simulated seconds per real second, frame rate, loop (`LvdViewPlaybackController`, a timer over the un-throttled `lvd_renderSceneAtTime`, which is now the body of the slider callback). **Export:** frame-exact MPEG-4 / Motion JPEG AVI / animated GIF video and PNG/JPG stills or clipboard copy (`LvdViewExporter`). **Camera:** four modes per view profile — Manual (today's mouse camera), Chase (fixed az/el/range offset from the vehicle, `LvdChaseCameraSettings`), Camera Script (`LvdCameraScript` of `LvdCameraKeyframe`s: each keyframe is scene-fixed or vehicle-relative, anchored to an absolute UT or to an event's start/end plus offset, held for a duration, and blended into the next with linear or smooth-step easing; deleting an anchored event freezes the keyframe at its absolute time rather than blocking the deletion) and Fixed Camera (Tracking) (a launch-pad/tracking-station camera that sits at a fixed anchor and keeps the vehicle centred with a fixed field of view, `LvdFixedAnchorCameraSettings`; the anchor is a fixed XYZ coordinate in a chosen reference frame — a body-fixed frame rotates with the planet like a real pad, an inertial one stays put — or one of the mission's ground objects or vehicle-independent geometric points, resolved into the view frame each frame; a "Set From Current Camera" button and, for the XYZ anchor, a mouse drag retune the anchor, while an object anchor drag takes manual control). `LvdSceneCameraDriver` applies the mode on every frame and keeps the saved manual camera untouched despite the main window's camera write-back listeners; the mouse always works (a drag retunes the chase offsets in Chase mode; in Camera Script mode it *detaches* the camera for authoring — scrub, aim, "Add Keyframe Here" / "Update Selected From Camera" / "Resume Script" — without leaving the mode; new keyframes default to vehicle-relative and anchored to the start of the active event, the keyframe table is in play order with the current keyframe in bold, and a row click goes to that keyframe). **Data overlay** (added the same day): any Graphical Analysis quantity, plus epoch / UT / mission elapsed time / current event header lines, drawn as a text block in a chosen corner of the 3-D view (and so into exported video) with per-quantity label, decimals, Fixed / Scientific / Auto format and units, font name/size/bold/colour and a background box (`LvdViewOverlaySettings`, `LvdViewOverlayItem`, `LaunchVehicleViewProfileOverlayData`; Data Overlay tab with search, frame selector, editable table and live preview). Every edit made in the window records a labelled Edit > Undo state and the window re-binds after an undo; the title, a banner and the status line name the **active view profile** the settings belong to. **Vehicle mesh:** STL (binary/ASCII) or OBJ import (`lvd_readMeshFile`, self-contained), geometry embedded in the mission with the source path for reload, uniform scale / fit-longest-side, yaw-pitch-roll rotation offset and translation to align the mesh with the body axes, colour / opacity / edges / lighting, a body-frame preview with an RGB axes triad; rendered per frame by `LaunchVehicleViewProfileVehicleMeshData` at the interpolated position with a re-orthonormalised body→view DCM. All settings live on `LaunchVehicleViewProfile` (`cameraMode`, `chaseCamera`, `cameraScript`, `fixedAnchorCamera`, `vehicleMesh`, `playbackSettings`, `overlay`), default to the old behaviour and are back-filled by `loadobj` for older missions. Tests: `CameraMathTest`, `FixedAnchorCameraTest`, `MeshFileReaderTest`, `VehicleMeshSettingsTest`, `CameraScriptTest`, `ViewProfileF8PersistenceTest`, `PlaybackControllerTest`, `ViewExporterTest`, `ViewOverlayTest`, `RenderSceneAtTimeTest` (real main window) and `ViewPlaybackGuiTest` (App Testing Framework gestures on the real main window and the new window).*
...
**G1. Case Matrix over any parameter.** ✅ **Implemented 2026-09-23.**
*Status: generalized sweep engine under `helper_methods/ksptot_lvd/classes/CaseMatrix/` — `AbstractLvdSweepParameter` with plugin-var, optimization-variable-element, constraint-bound and vehicle-knob kinds (`LvdSweepParameterFactory.enumerate`), grid + uniform/normal/triangular variations, full-factorial / Latin-hypercube / random sampling (`LvdSweepSampler`), propagate-only + optimize run modes with parallel-chunk and serial-no-pool dispatch, `LvdSweepResponse` harvesting any GA quantity at any event node, `LvdSweepSetup` persisted on `LvdData.caseMatrixSetup`, `LvdSweepResults` with statistics/covariance/Excel/MAT/CSV output. `lvd_runCaseMatrix_App.mlapp` rebuilt around it (Parameters / Sampling and Run / Responses / Output / Status tabs, canvas components); results viewed in `lvd_SweepResultsGUI_App` (data table, scatter/carpet, histogram/CDF/percentiles, 1/2/3σ error ellipses). Tests: `SweepParameterTest`, `SweepDistributionTest`, `SweepSamplerTest`, `SweepResponseTest`, `SweepEngineTest`, `CaseMatrixTaskIoTest`, `CaseMatrixGuiTest`, `SweepResultsGuiTest`.*
...
**G2. Monte Carlo dispersion analysis.** ✅ **Implemented 2026-09-23.**
*Status: `lvd_runMonteCarlo_App.m` (programmatic window, Simulation menu) over the same engine — dispersion table with distributions defaulted around current values, N/seed/workers, response table, run/cancel/progress, Open Results into the shared viewer. Disperses plugin variables, optimization-variable elements and vehicle knobs; constraint bounds are Case Matrix only (meaningless propagate-only) and wind awaits D1. Setup persists on `LvdData.monteCarloSetup`. **Optimize mode added later the same day:** each case re-runs the optimizer (warm-started, serial-forced, pool required) with live Iter/Objective/MaxViol/Optimality columns and final objective + exit status harvested per case — the dispersion-of-the-optimum analysis (e.g. dry mass vs. maximized payload). Tests: `MonteCarloGuiTest`, `SweepOptimizeRunTest`.*
