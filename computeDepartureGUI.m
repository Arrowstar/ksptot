...
handles.mainGUIFigure = mainGUIFigure;
guidata(hObject, handles);
...
if(isempty(errMsg))
    try
        hFig = computeDeparture(userDataMain{1,1}, departBody, arrivalBody, departUT, arrivalUT, eSMA, eEcc, eInc, eRAAN, eArg, gmuXfr, cbName, options, eMA, eEpoch);
...
if(isempty(errMsg))
    objFunc = @(arrivalTime) findOptimalDepartureArrivalObjFunc(arrivalTime, departUT, departBodyInfo, arrivalBodyInfo, gmu, options.quant2opt, 0);
    atLB = departUT+1000;
...
if(isempty(errMsg))
    ...
    objFunc = @(departUT) findOptimalDepartureArrivalObjFunc(arrivalUT, departUT, departBodyInfo, arrivalBodyInfo, gmu, options.quant2opt, 0);
    dtUB = arrivalUT-100;
...
if(isempty(errMsg))
    ...
    dvFunc = @(arrivalUT, departUT) findOptimalDepartureArrivalObjFunc(arrivalUT, departUT, departBodyInfo, arrivalBodyInfo, gmu, options.quant2opt, 0);
