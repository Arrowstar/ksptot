...
function initializeOutputWindowText(handles, hOutputText) 
    ...
    statusBoxMsg = {['KSP TOT Launch Vehicle Designer v', getKSPTOTVersionNumStr(), sprintf(' (R%s)', version('-release'))], 'Written By Arrowstar (C) 2023', ...
...
%     ma_CelBodyCatalogGUI(lvdData.celBodyData);
    ma_CelBodyCatalogGUI_App(lvdData.celBodyData);
...
%     lvd_viewSettingsGUI(lvdData.viewSettings);
    hFig = lvd_viewSettingsGUI_App(lvdData.viewSettings);
    delete(hFig);
...
% --------------------------------------------------------------------
function haloOrbitConstructorMenu_Callback(hObject, eventdata, handles)
% hObject    handle to haloOrbitConstructorMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    lvdData = getappdata(handles.ma_LvdMainGUI,'lvdData');
