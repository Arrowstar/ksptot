classdef CaseMatrixPluginGroundObjTest < KsptotTestCase
    ...
    methods(Access=private)
        ...
        function checkCaseMatrixPrereqsCancelAndUiTable(testCase)
            ...
            %parameter, the optimize summary (Iter, Objective, Max Viol,
            %Optimality, Exit -- NaN/blank here since nothing optimized),
            %then status name and message.
            ...
            testCase.verifyEqual(size(data), [4 10], ...
                'The UI table should be one row per task and one column for the index, each swept parameter, the five optimize-summary columns, the status and the message.');
            for(i = 1:4)
                ...
                testCase.verifyTrue(isnan(data{i,4}) && isnan(data{i,5}) && isnan(data{i,6}), ...
                    sprintf('UI table row %u should carry NaN optimize summaries: nothing optimized.', i));
                testCase.verifyEqual(data{i,7}, '', ...
                    sprintf('UI table row %u should carry a blank optimality: nothing optimized.', i));
                testCase.verifyEqual(data{i,8}, '', ...
                    sprintf('UI table row %u should carry a blank exit tag: nothing optimized.', i));
                testCase.verifyEqual(data{i,9}, caseMatrix3.tasks(i).status.name, ...
                    sprintf('UI table row %u does not carry that task''s status name.', i));
                testCase.verifyEqual(data{i,10}, caseMatrix3.tasks(i).taskOutputMessage, ...
                    sprintf('UI table row %u does not carry that task''s output message.', i));
            end
            testCase.verifyEqual(data{1,9}, 'Completed', ...
                'The completed task''s UI row does not say Completed.');
            testCase.verifyEqual(data{2,9}, 'Not Run', ...
