...
% Last Modified by GUIDE v2.5 22-Apr-2015 16:52:42
...
function populateGUIFromOptSettings(handles, maData)
    ...
    if(~isempty(maData.optimizer.objective))
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if(~isempty(selectedCelBody))
        bodyID = celBodyData.(lower(selectedCelBody)).id;
    else
        bodyID = -1;
    end
...
%     for(i=1:size(optVarData,1))
    ...
    objFuncWrapper = @(x) ma_optimObjWrapper(x, maData.script,vars,objFunc,celBodyData, partialExec);
    ...
    for(i=1:size(actConsts,1))
        ...
        constFuncts{end+1} = buildConstraints(handles, constData(3), constData(4), constData(7), bodyName, otherSC, actConsts{i,3}, lbActive, ubActive); %#ok<AGROW>   
    end
    
    if(isempty(constFuncts))
        nonlcon = [];
    else
        nonlcon = @(x) ma_optimConstrWrapper(x, maData.script,vars,constFuncts,celBodyData);
    end
    ...
    %%%%%%%
    options = optimoptions('fmincon','Algorithm','interior-point','Display','iter','ScaleProblem','obj-and-constr','MaxIter',500,'UseParallel',usePara, 'OutputFcn', [], 'InitBarrierParam',0.75,'InitTrustRegionRadius',nthroot(length(vars),3),'AlwaysHonorConstraints','none');
