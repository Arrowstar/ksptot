...
handles.dispAxes = handles.movieAxes;
...
function handles = initGUI(handles)
    ...
    setCamSrcTgtComboBoxes(handles, 0);
    ...
    showEventNameCheckbox_Callback(handles.showEventNameCheckbox, [], handles);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    jumpToUT = -1;
    for(i=length(bodyChangeInd):-1:1) %#ok<*NO4LP>
...
%        contents{get(hObject,'Value')} returns selected item from cameraSrcCombo
    setSrcComboUserData(hObject, handles, true);
...
%        contents{get(hObject,'Value')} returns selected item from cameraTgtCombo
    setTgtComboUserData(hObject, handles, true);
...
% handles    structure with handles and user data (see GUIDATA)
    switch(eventdata.Key)
        ...
        case 'space'
            if(~isa(hObject.CurrentObject,'matlab.ui.control.UIControl') || (isa(hObject.CurrentObject,'matlab.ui.control.UIControl') && ~strcmpi(hObject.CurrentObject.Style,'edit') && ...
                                                                                                                                         ~strcmpi(hObject.CurrentObject.Style,'checkbox') && ...
                                                                                                                                         ~strcmpi(hObject.CurrentObject.Style,'pushbutton') && ...
                                                                                                                                         ~strcmpi(hObject.CurrentObject.Style,'togglebutton')))
                if(get(handles.playButton,'Value')==1)
                    set(handles.playButton,'Value',0);
                elseif(get(handles.playButton,'Value')==0)
                    set(handles.playButton,'Value',1);
                end
                playButton_Callback(handles.playButton, [], handles);
            end
...
% --- Executes on button press in showEventNameCheckbox.
function showEventNameCheckbox_Callback(hObject, eventdata, handles)
% hObject    handle to showEventNameCheckbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of showEventNameCheckbox
    set(hObject,'UserData',get(hObject,'Value'));
    if(get(handles.playButton,'Value')==0)
        plotOneFrame(handles);
    end
