...
% Last Modified by GUIDE v2.5 22-Apr-2015 16:56:07
...
handles = createGUIWidgets(handles, numBurns, totalDeltaV);
slidersCallback(-1, [], handles.splitDvPanel, numBurns, totalDeltaV);
guidata(hObject, handles);
...
function handles = createGUIWidgets(handles, numBurns, totalDdeltaV)
    deltaY = 30;
    ...
    dvStr = strtrim(sprintf('%8.3f',totalDdeltaV));
    titleDV = sprintf('Split Delta-V Maneuver (%s m/s)',dvStr);
    set(handles.splitDvPanel,'Title',titleDV);
    
    for(i=1:numBurns) %#ok<*NO4LP>
        sCb = @(src,evt) slidersCallback(src, evt, handles.splitDvPanel, numBurns, totalDdeltaV);
...
function slidersCallback(src, ~, parent, numBurns, totalDeltaV)      
...
% Get default command line output from handles structure
    if(isempty(handles))
        ...
        for(i=1:numBurns)
            ...
            burnValues(end+1,:) = [i,slider.Value];
        end
