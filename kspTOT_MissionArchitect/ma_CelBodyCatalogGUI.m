...
%        contents{get(hObject,'Value')} returns selected item from catalogListbox
    ...
    elseif(dispOrbValue==1)
        if(~isempty(parentInfo))
            plotBodyOrbit(bodyInfo, 'r', parentInfo.gm);
...
function plotAtmoCurve(orbitDispAxes, bodyInfo)
	p0 = bodyInfo.atmopresssl; %kPa
    scaleHgt = bodyInfo.atmoscalehgt; %km
    atmoHgt = bodyInfo.atmohgt;
    
    if(atmoHgt > 0 && scaleHgt > 0 && p0 > 0)
        pressCurve = @(h) p0 .* exp(-h./scaleHgt);

        hArr = linspace(0,atmoHgt,100);
        press = pressCurve(hArr);

        plot(orbitDispAxes,hArr,press,'r-','LineWidth',2);
        grid on;
...
function bodyInfoStr = generateBodyInfoStr(bodyInfo, celBodyData)
    ...
    if(bodyInfo.sma > 0 && ~isempty(parentInfo))
        ...
        period = computePeriod(bodyInfo.sma, parentInfo.gm);
    ...
    bodyInfoStr{end+1} = [' Atmo SL Press = ', num2str(bodyInfo.atmopresssl), ' kPa'];
    bodyInfoStr{end+1} = [' Atmo Scale Hgt = ', num2str(bodyInfo.atmoscalehgt*1000), ' m'];
